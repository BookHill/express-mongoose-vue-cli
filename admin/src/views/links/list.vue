    ...
    <div class="wrapperVue">
        ...
        <div class="wrapper">
            ...
            <div class="clearfix wrapper-nav">
                <ul class="left">
                    <li>
                        <router-link class="navlink" :to="'/home/links/add'">
                ...
                <tr v-for="(table, index) in tables">
                    ...
                    <td>
                        <router-link class="handleEdit" 
                            :to="'/home/links/'+(index+(page.current-1)*10)">编辑
                        </router-link>
                        <span class="vertical-line">|</span>
                        <a class="handleDelete" 
                            @click="deletePost(table.name, table.url)">删除
                        </a>
                    </td>
                </tr>
                <tr v-show="NoData">
                    <td colspan="4"><img src="@/assets/NoData.png" alt="暂无数据"/></td>
                </tr>
            </table>
            <!-- page -->
            <div class="pageBox">
...
<script>
    export default {
        ...
        created() {
            this.linksPost(this.page.current);
        },
        methods: {
            linksPost(page) {
                this.$http.post('/api/users/links/find', {
            ...
            deletePost(key, value) {
                ...
                this.$popup.prompt( () => {                 // callback
                    this.$http.post('/api/users/links/delete', {
                        params: { key, value }
                    }).then( (response) => {
                        let res = response.data;
                        if ( res.session==true && res.status==1 ) {
