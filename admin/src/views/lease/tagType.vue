    ...
    <div class="wrapperVue">
        ...
        <div class="wrapper">
            ...
            <div class="clearfix wrapper-nav">
                <ul class="left">
                    <li>
                        <router-link class="navlink" :to="'/home/lease/type/tag/add'">
                ...
                <tr v-for="(table, index) in tables">
                    ...
                    <td>
                        <router-link class="handleEdit" 
                            :to="'/home/lease/type/tag/'+(index+(page.current-1)*10)">编辑
                        </router-link>
                        <span class="vertical-line">|</span>
                        <a class="handleDelete" 
                            @click="deletePost(table)">删除
                        </a>
                    </td>
                </tr>
                <tr v-show="NoData">
...
<script>
    export default {
        ...
        methods: {
            listPost(page) {
                this.$http.post('/api/users/leaseType/tag/find', {
                    params: { page }
                }).then( (response) => {
            ...
            deletePost(id) {
                ...
                this.$popup.prompt( () => {                 // callback
                    this.$http.post('/api/users/leaseType/tag/delete', {
                        params: { id }
                    }).then( (response) => {
                        let res = response.data;
                        if ( res.session==true && res.status==1 ) {
